...
- 2、主内容区改为文件夹化管理，支持在当前目录内直接新建文件夹、添加书签、编辑和删除
- 3、`常用推荐` 替代旧置顶逻辑，普通分类书签支持星标加入/移出常用推荐
- 4、删除或编辑分类时，已登录状态不再重复输入密码，仅保留必要确认
- 5、批量编辑改为 hover 复选框触发，批量移动、复制、删除交互更直接
- 6、搜索改为本地模糊匹配优先，无结果时自动走站外搜索；支持默认搜索源管理
- 7、主题支持浅色 / 深色 / 跟随系统，整体界面、详情卡片、弹窗与侧栏排版统一优化
- 8、修正开发预览端口和启动脚本，默认使用 `3000` 并输出 `CloudNav/.dev-server-info.json`
